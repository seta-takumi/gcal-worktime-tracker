...
- **終日予定の自動作成**: 設定した週数分（デフォルト 1 週間）の平日に `🧑‍💻 作業可能 6h30m` 形式の終日予定を自動生成（カレンダーを開くたびに上書き更新）
...
- 設定した**除外ワード**をタイトルに含む予定は計算から除外する（差し引かない）
...
- `calendar.google.com` を開くと自動的に今週から設定した週数分の終日予定が生成される（今週は今日以降の日のみ、翌週以降は全日対象）
...
| 集計週数 | 今週から何週間分の終日予定を書き込むか（1〜4 週間） | 1週間 |
| 開始時刻 | 就業開始時刻 | 10:00 |
| 終了時刻 | 就業終了時刻 | 19:00 |
| 除外ワード | タイトルにこのワードを含む予定を計算から除外する（1 行 1 ワード、大文字小文字を区別しない部分一致） | なし |
